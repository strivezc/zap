...
Zap 是基于 [Warp](https://github.com/warpdotdev/warp) 开源代码的社区分支 ——
保留 Warp 全部终端体验,同时**剥离对 Warp 云端的依赖**,
开放 AI 接入层、第三方 Agent 接入、SSH 管理与渲染细节,
让密钥、模型、提示词、Agent、会话历史 **全部留在本地**。

[English](./README.md) · [上游 Warp](https://www.warp.dev) · [上游同步说明](docs/zap-upstream-sync.md)
...
Zap 把这层完全打开,并在上游基础上**追加**了一批上游不支持的能力:
...
以下能力上游 Warp **不提供**,是 Zap 在 fork 之上新增的差异化功能:
...
- **第三方 CLI Agent 接入** —— 可把任意 CLI Agent 接入终端 Block 体系,内置适配:
  ...
  - 通过 OSC9 / OSC777 通知机制统一进入 Zap 通知中心
...
**03 · 在终端立即启用**
一键切换模型、对话、命令补全、第三方 Agent —— 体验与上游 Warp 一致,但每一层都由你掌控。
...
- **流式打字机** — SSE 增量渲染,与上游一致的 Block 体验
...
- **保留 Warp 终端体验** — 持续合并上游,Blocks / Workflows / AI 命令 / Keymaps / 主题完整保留
...
`zerx-lab/zap` 维护两条长期分支:

| 分支 | 跟踪 | 用途 |
| --- | --- | --- |
| `main` | `zerx-lab/zap:main`(默认分支) | Zap 主开发线,**所有 PR 都提到这条分支**。 |
...
**不要在 GitHub 网页上点 `main` 分支的 "Sync fork" 按钮**。这会把上游整段历史直接合并进 Zap 主线,造成大规模冲突。拉取上游更新请通过镜像分支,并参考 [`docs/zap-upstream-sync.md`](docs/zap-upstream-sync.md) 中的黑名单与流程:
...
终端 UI。Zap 提供一流的接入支持:完成通知、OSC9 文本通知映射、输入框恢复
链路均已打通,DeepSeek-TUI 可作为原生 Block 在 Zap 中运行。

在任意 Zap 终端中执行 `deepseek` 即可启动,Block 生命周期、底部状态条与
通知中心全部开箱即用。
...
> Zap 将自身标记为 `WarpTerminal`,因此 Windows 下要让 DeepSeek-TUI 的
> 完成通知进入 Zap 通知中心,需要在 `~/.deepseek/config.toml` 中显式指定:
>
> ```toml
> [notifications]
> method = "osc9"
...
提交 Issue 前,请先 [搜索现有 Issues](https://github.com/zerx-lab/zap/issues)。
